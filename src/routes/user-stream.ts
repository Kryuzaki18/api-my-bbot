...
const KeepAliveStreamSchema = {
  description: 'Pings a listenKey to keep it alive. Should be called every 30 minutes.',
  tags: ['User Stream'],
  security: [{ bearerAuth: [] }],
  response: {
    200: Type.Any(),
...
const userStreamRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.post(ROUTES.USER_STREAM, { schema: GetStreamSchema }, async (request, reply) => {
    try {
  ...
  fastify.put(ROUTES.USER_STREAM, { schema: KeepAliveStreamSchema }, async (request, reply) => {
    try {
      const { apiKey, useTestnet } = request.user;
      const result = await binanceService.keepAliveListenKey(apiKey, useTestnet);
      return reply.code(200).send(result);
    } catch (error: any) {
      if (error.status) return reply.code(error.status).send({ error: error.message, details: error.details });
      return reply.code(500).send({ error: 'Internal Server Error', details: error.message });
  ...
  fastify.delete(ROUTES.USER_STREAM, { schema: CloseStreamSchema }, async (request, reply) => {
    try {
      const { apiKey, useTestnet } = request.user;
